...
__global__ void GOL33_gm(uint64_t* grid, uint64_t* newGrid, int* GPU_lookup_table, int ROW_SIZE, int GRID_SIZE, int horizontalHaloWidth, int verticalHaloSize) {
    ...
    if (iy >= verticalHaloSize && iy < GRID_SIZE + verticalHaloSize && ix >= horizontalHaloWidth && ix < ROW_SIZE + horizontalHaloWidth) {
        for (int i = -RADIUS; i <= RADIUS; i++) {
            uint64_t currentNeighCellIndex = (uint64_t) (iy+i) * (uint64_t) fullHorizontalSize + (uint64_t) ix;
            left[0] = grid[currentNeighCellIndex - 1];
            centerWord = grid[currentNeighCellIndex];
            ...
            for (int j = -RADIUS; j < 0; j++) {
                subcell = getSubCellD(left[((-j)-1) >> 3], (j) & (ELEMENTS_PER_CELL - 1));
                for (int k = max(0, j-RADIUS); k <= min(7, j+RADIUS); k++) {
                        subcells[k] += subcell;
                }
            }
            // CENTER 
            #pragma unroll
            for (int j = 0; j < 8; j++) {
                subcell = getSubCellD(centerWord, j);
				if (i==0){
                    // subcells left of `j`-th subcell
					for (int k = max(0, j-RADIUS); k <= j-1; k++) {
						subcells[k] += subcell;
					}
                    // subcells right of `j`-th subcell
					for (int k = j+1; k <= min(7, j+RADIUS); k++) {
						subcells[k] += subcell;
					}
				} else {
                    // i /= 0 --> no need to check
					for (int k = max(0, j-RADIUS); k <= min(7, j + RADIUS); k++) {
						subcells[k] += subcell;
					}
				}
            ...
            for (int j = 8; j < 8 + RADIUS; j++) {
                subcell = getSubCellD(right[(j-8) >> 3], (j) & (ELEMENTS_PER_CELL - 1));
                for (int k = max(0, j-RADIUS); k <= min(7, j + RADIUS); k++) {
        		    subcells[k] += subcell;
                }
            }
