...
#include "Debug.h"
template <typename T> class CATWithoutCAT : public GPUSolver<T>
{
  protected:
  ...
  public:
    CATWithoutCAT(int deviceId, CADataDomain<T> *deviceData, CADataDomain<T> *deviceDataBuffer)
        : GPUSolver<T>(deviceId, deviceData, deviceDataBuffer)
    {
        ...
        this->createStream();
        cudaMemset(this->dataDomainBufferDevice->getData(), 0,
                   sizeof(half) * this->dataDomainBufferDevice->getTotalSize());
    };
};

#include "CellularAutomata/Solvers/CATWithoutCAT.tpp"