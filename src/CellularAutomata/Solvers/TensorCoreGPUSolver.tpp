...
void TensorCoreGPUSolver<T>::moveCurrentDeviceStateToGPUBuffer() {
    convertFp16ToFp32<<<this->castingKernelsGridSize, this->castingKernelsBlockSize>>>(this->visibleDataDevice->getData(), this->dataDomainDevice->getData(), this->dataDomainDevice->getSideLength());
}

template <typename T>
void TensorCoreGPUSolver<T>::moveGPUBufferToCurrentDeviceState() {
    convertFp32ToFp16<<<this->castingKernelsGridSize, this->castingKernelsBlockSize>>>(this->dataDomainDevice->getData(), this->visibleDataDevice->getData(), this->dataDomainDevice->getSideLength());
}

template <typename T>
void TensorCoreGPUSolver<T>::fillHorizontalBoundaryConditions() {
    int n = this->dataDomainDevice->getSideLengthWithoutHalo();
    copyHorizontalHaloTensor<<<this->horizontalBoundaryGrid, this->boundaryBlock>>>(this->dataDomainDevice->getData(), n, this->dataDomainDevice->getSideLength());
}

template <typename T>
void TensorCoreGPUSolver<T>::fillVerticalBoundaryConditions() {
    int n = this->dataDomainDevice->getSideLengthWithoutHalo();
    copyVerticalHaloTensor<<<this->verticalBoundaryGrid, this->boundaryBlock>>>(this->dataDomainDevice->getData(), n, this->dataDomainDevice->getSideLength());
}

template <typename T>
void TensorCoreGPUSolver<T>::CAStepAlgorithm() {
    int n = this->dataDomainDevice->getSideLengthWithoutHalo();
    TensorV1GoLStep<<<this->GPUGrid, this->GPUBlock, sharedMemoryBytes, mainStream>>>(this->dataDomainDevice->getData(), this->dataDomainBufferDevice->getData(), n, this->dataDomainDevice->getSideLength());
