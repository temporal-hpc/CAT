...
class FastTensorCoreGPUSolver : public GPUSolver<T> {
   ...
   public:
    FastTensorCoreGPUSolver(int deviceId, CADataDomain<T>* deviceData, CADataDomain<T>* deviceDataBuffer) : GPUSolver<T>(deviceId, deviceData, deviceDataBuffer) {
        this->setupBlockSize();
        this->setupGridSize();
...
#include "CellularAutomata/Solvers/FastTensorCoreGPUSolver.tpp"