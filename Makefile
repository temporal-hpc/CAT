...
GLOBALDEFINES := -Iinclude/ -Isrc/ -w -DRADIUS=${RADIUS} -DSMIN=${SMIN} -DSMAX=${SMAX} -DBMIN=${BMIN} -DBMAX=${BMAX} -DBSIZE3DX=${BSIZE3DX} -DBSIZE3DY=${BSIZE3DY} -DBSIZE3DZ=${BSIZE3DZ} -D${MEASURE_POWER} -DNREGIONS_H=${NREGIONS_H} -DNREGIONS_V=${NREGIONS_V} -D${MEASURE_POWER} 
CCDEFINES :=
CUDEFINES :=
